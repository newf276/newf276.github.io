...
ORDER = ['exodusredux', 'fen', 'gaia', 'numbers', 'openmeta', 'premiumizer',
         'realizer', 'scrubs', 'seren', 'shadow', 'thecrew', 'trakt', 'penguin']
...
TRAKTID = {
    ...
    'penguin': {
        ...
        'file'     : os.path.join(CONFIG.TRAKTFOLD, 'penguin_trakt'),
        'settings' : os.path.join(CONFIG.ADDON_DATA, 'plugin.video.penguin', 'settings.xml'),
        'default'  : 'trakt.user',
        'data'     : ['trakt.token', 'trakt.refresh', 'trakt.user'],
        'activate' : 'RunPlugin(plugin://plugin.video.penguin/?action=authTrakt&opensettings=tru&query=10.2)'},
    'thecrew': {
        'name'     : 'THE CREW',
    ...
    'finder': {
        ...
        'file'     : os.path.join(CONFIG.TRAKTFOLD, 'finder_trakt'),
        ...
        'activate' : 'RunScript(plugin.video.finder, authenticate_trakt)'},
    'fen': {
        'name'     : 'Fen',
