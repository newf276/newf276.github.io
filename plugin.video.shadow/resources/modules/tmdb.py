...
def get_tmdb_data(new_name_array,html_g,fav_search_f,fav_servers_en,fav_servers,google_server,rapid_server,direct_server,heb_server,url,isr,xxx):
       ...
       #except:
           ...
           for data in html['results']:
            ...
            xbmc.executebuiltin((u'Notification(%s,%s)' % (Addon.getAddonInfo('name'), 'No Trailer...Line:'+str(lineno)+' E:'+str(e))).encode('utf-8'))
...
def get_movies(url,local=False,reco=0,global_s=False):
   ...
   if '/search' in url and 'page=1' in url:
       ...
       if search_entered+'$$$'+tv_movie not in all_s_strings:
         dbcur.execute("SELECT * FROM search_string2 where name='%s' and tv_movie='%s'"%(urllib.unquote_plus(search_entered.replace("'","%27")),tv_movie))
         match = dbcur.fetchall()
   ...
   if max_page>int(match[0]):
        if local:
            ...
            mode=14
        aa=addDir3(('[COLOR orange][I]%s %s %s %s (%s %s)[/I][/COLOR]'%(Addon.getLocalizedString(32184),str(int(match[0])+1),Addon.getLocalizedString(32185),str(max_page),str(all_res).decode('utf8'),Addon.getLocalizedString(32186))),link+'page='+str(int(match[0])+1),mode,BASE_LOGO+'next.png','https://cdn4.iconfinder.com/data/icons/arrows-1-6/48/1-512.png','Results'.decode('utf8'),show_original_year='999',data='999',collect_all=True)
        all_d.append(aa)
...
def get_seasons(name,url,iconimage,fanart,description,data,original_title,id,heb_name):
   ...
   for data in html['seasons']:
     ...
     new_name=Addon.getLocalizedString(32101).decode('utf8')+str(data['season_number'])
