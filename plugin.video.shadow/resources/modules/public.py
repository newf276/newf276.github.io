...
def addDir3(name,url,mode,iconimage,fanart,description,premired=' ',image_master='',all_w_trk='',last_id='',video_info={},data=' ',original_title=' ',id=' ',season=' ',episode=' ',tmdbid=' ',eng_name=' ',show_original_year=' ',rating=0,heb_name=' ',isr=0,generes=' ',trailer=' ',dates=' ',watched='no',fav_status='false',collect_all=False,ep_number='',watched_ep='',remain='',hist='',join_menu=False,menu_leave=False,remove_from_fd_g=False,all_w={},mark_time=False,ct_date=''):
        ...
        if len(id)>1:
            ...
            if Addon.getSetting("trakt_manager")=='true':
                menu_items.append((Addon.getLocalizedString(32170), 'XBMC.RunPlugin(%s)' % ('%s?url=%s&mode=150&name=%s&data=%s')%(sys.argv[0],id,original_title,tv_mov) ))
            if Addon.getSetting("trakt_watched")=='true':
                menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32171), 'XBMC.RunPlugin(%s)' % ('%s?url=www&original_title=add&mode=65&name=%s&id=%s&season=%s&episode=%s')%(sys.argv[0],tv_show,id,season,episode))) 
            if Addon.getSetting("trakt_unwatched")=='true':
                menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32172), 'XBMC.RunPlugin(%s)' % ('%s?url=www&original_title=remove&mode=65&name=%s&id=%s&season=%s&episode=%s')%(sys.argv[0],tv_show,id,season,episode))) 
        ...
        if mark_time:
            if Addon.getSetting("remove_resume_time")=='true':
                menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32173), 'XBMC.RunPlugin(%s)' % ('%s?url=www&mode=160&name=%s&id=%s&season=%s&episode=%s&data=%s')%(sys.argv[0],name,id,season,episode,tv_movie))) 
        if mode==15:
        ...
        if mode==15:
            ...
            if Addon.getSetting("s3d_scrape")=='true':
                ...
                menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32178), 'XBMC.RunPlugin(%s)' % ('%s?%s')%(sys.argv[0],all_ur2))) 
        liz.addContextMenuItems(menu_items, replaceItems=False)
        params={}
...
def addLink( name, url,mode,isFolder, iconimage,fanart,description,place_control=False,data='',rating='',generes='',no_subs='0',tmdb='0',season='0',episode='0',original_title='',da='',year=0,all_w={},dd='',in_groups=False):
          ...
          if Addon.getSetting("set_view_type")=='true':
            menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32179), 'XBMC.RunPlugin(%s)' % ('%s?url=%s&mode=167')%(sys.argv[0],str(pre_mode))))
          if mode==170:
            menu_items.append(('[I]%s[/I]'%Addon.getLocalizedString(32180), 'XBMC.RunPlugin(%s)' % ('%s?name=%s&url=www&id=%s&mode=171')%(sys.argv[0],name,tmdb)))
          liz.addContextMenuItems(menu_items, replaceItems=False)
          if in_groups:
