...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if 1:
        ...
        for items in x['episodes']:
                         ...
                         try_head = get_html(link2[0].replace('\\',''),headers=base_header,cookies=cookies, stream=True,verify=False,timeout=15).headers()
                         size=0
                         logging.warning(try_head)
                         if 'Content-Length' in try_head:
          
                            if int(try_head['Content-Length'])>(1024*1024):
                                size=float(try_head['Content-Length'])/(1024*1024*1024)
