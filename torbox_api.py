...
from modules.kodi_utils import confirm_dialog, kodi_dialog, make_session, notification, ok_dialog
from modules.source_utils import extras, seas_ep_filter, supported_video_extensions
...
class TorBoxAPI:
	def __init__(self):
		self.token = get_setting("finder.tb.token")
	...
	def _get(self, url, data={}):
		...
		response = session.get(url, params=data, headers=headers, timeout=TIMEOUT_STANDARD)
		return response.json()

	def _post(self, url, params=None, json=None, data=None):
		if self.token in ("empty_setting", "") and "token" not in url:
			return None
		headers = {"Authorization": "Bearer %s" % self.token}
		url = "https://api.torbox.app/v1/api/" + url
		response = session.post(url, params=params, json=json, data=data, headers=headers, timeout=TIMEOUT_STANDARD)
		return response.json()
