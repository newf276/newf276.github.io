	...
	def display_results(self, page_no=None, previous_nav=None):
		...
		def _build_directory():
			for item in results:
				...
				poster = meta['poster']
				logger('poster', poster)
				# listitem.setArt({'poster': meta['poster'], 'fanart': meta['fanart'], 'icon': meta['poster'], 'banner': meta['banner'], 'clearart': meta['clearart'], 'clearlogo': meta['clearlogo'], 'landscape': meta['landscape'], 'discart': meta['discart']})
		...
		if paginate_results and results:
			if limit == len(results):
				...
				listitem.setArt({'icon': os.path.join(settings.get_theme(), 'item_next.png'), 'fanart': meta['fanart'], 'clearlogo': meta['clearlogo']})
				listitem.setInfo('video', {})
