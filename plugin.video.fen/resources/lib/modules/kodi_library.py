...
def retrieve_kodi_library(db_type, page_no, letter):
	...
	r = sorted(r, key=lambda k: title_key(k['title'], ignore_articles))
	...
	if settings.paginate(): final_list, total_pages = paginate_list(original_list, page_no, letter, limit)
...
def build_kodi_library_recently_added(db_type):
	...
	limit = int(get_setting('recent_added_limit'))
	recent_added = []
	results = []
	if db_type in ('movie', 'movies'):
		...
		try:
			...
			Movies({'list': results, 'id_type': 'imdb_id'}).worker()
		except: pass
	else:
		...
		def process_eps(item):
			episode_item = {"season": int(item['season']), "episode": int(item['episode']), 'order': item['order'], "meta": tvshow_meta('tvdb_id',
										  item['media_id'], meta_user_info)}
			results.append(build_episode(episode_item, watched_info, use_trakt, meta_user_info, meta_user_info_json, is_widget))
		try:
			db_type = 'episodes'
