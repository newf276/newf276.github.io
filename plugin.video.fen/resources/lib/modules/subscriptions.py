...
from modules.utils import adjust_premiered_date, to_utf8, clean_file_name, confirm_dialog
...
class Subscriptions:
	...
	def add_remove(self, silent=False):
		...
		if self.check_exists() and self.action == 'add':
			return notification(ls(32833) % self.rootname, 4500)
	...
	def clear_subscriptions(self, confirm=True, silent=False, db_type=None):
		...
		if len(subscriptions) == 0:
			return notification(ls(32834) % self.db_type.upper(), 4500)
		self.remove_folder(self.path)
		self.make_folder(self.path)
		if not silent: notification(ls(32835) % self.db_type.upper(), 4500)
	...
	def update_subscriptions(self, suppress_extras=False):
		...
		if not subscriptions:
			return notification(ls(32836), 1200)
		close_all_dialog()
		bg_dialog = xbmcgui.DialogProgressBG()
		bg_dialog.create(ls(32577), ls(32837))
		for count, item in enumerate(subscriptions, 1):
			try:
				self.tmdb_id = item['media_id']
				self.add_remove_constants()
				self.add_remove_tvshow(silent=True)
				display = ls(32838) % self.rootname
				bg_dialog.update(int(float(count) / float(len(subscriptions)) * 100), ls(32577), display)
				xbmc.sleep(300)
			except Exception as e:
				logger('update subscriptions error', e)
				logger('title', item['title'])
		if int(get_setting('subsciptions.update_type')) == 0:
			hours = settings.subscription_timer()
			set_setting('service_time', str(datetime.now() + timedelta(hours=hours)).split('.')[0])
		xbmc.sleep(500)
		bg_dialog.close()
		notification(ls(32839), 4500)
	...
	def add_trakt_subscription_listitem(self, db_type, ids, count, total, path, dialog):
		try:
			...
			dialog.update(int(float(count) / float(total) * 100), '', ls(32840) % rootname)
		except Exception as e:
	...
	def remove_trakt_subscription_listitem(self, db_type, media_id, count, total, path, dialog):
		try:
			...
			dialog.update(int(float(count) / float(total) * 100), ls(32577), ls(32841) % rootname)
		except: pass
	...
	def create_tvshow_strm_files(self, meta=None, folder=None, tmdb_id=None, title=None, year=None):
		...
		if not get_setting('subscriptions.include_specials', 'false') == 'true': season_data = [i for i in season_data if not i['season_number'] == 0]
	...
	def add_remove_constants(self):
		...
		self.notify = ls(32842) if self.action == 'add' else ls(32843)
...
def retrieve_subscriptions(db_type, page_no, letter):
	...
	original_list = sorted(data, key=lambda k: title_key(k['title'], ignore_articles))
...
def subscriptions_add_list(db_type):
	from caches.trakt_cache import clear_trakt_list_data
	...
	variables = (ls(32028), 'movies', 'movie', 'trakt.subscriptions_movie', 'trakt.subscriptions_movie_display', '9.14', movie_subscriptions) if db_type == 'movie' else (ls(32029), 'shows', 'show', 'trakt.subscriptions_show', 'trakt.subscriptions_show_display', '9.15', tvshow_subscriptions)
	...
	if xbmcgui.Dialog().yesno('Fen', ls(32682) % db_type.upper(), '', '', ls(32683), ls(32684)):
	...
	set_setting(main_setting, json.dumps(trakt_list))
	set_setting(display_setting, trakt_list['name'])
	if trakt_list['name'].lower() == 'none': return open_settings(open_setting)
	if not confirm_dialog():
		set_setting(main_setting, '')
		set_setting(display_setting, 'none')
	...
	if len(current_subscriptions) > 0:
		if not xbmcgui.Dialog().yesno('Fen', ls(32685) % (dialog_display)):
			return open_settings(open_setting)
		clear_library = True
		show_busy_dialog()
	...
	dialog.create(ls(32577), ls(32844))
	if trakt_list['name'] in ('Collection', 'Watchlist'):
		...
		from indexers.trakt_lists import get_trakt_list_contents
	...
	dialog.close()
	end_dialog = ls(32736) if cancelled else ls(32842)
	xbmcgui.Dialog().ok('Fen', end_dialog % trakt_list['name'])
	if clear_library:
		if xbmcgui.Dialog().yesno('Fen', ls(32686)):
			import time
			xbmc.executebuiltin('CleanLibrary(video)')
			time.sleep(3)
			while xbmc.getCondVisibility("Window.IsVisible(ProgressDialog)"):
				time.sleep(1)
	if xbmcgui.Dialog().yesno('Fen', ls(32687)):
...
def subscriptions_update_list():
	...
	close_all_dialog()
	dialog = xbmcgui.DialogProgressBG()
	dialog.create(ls(32577), ls(32837))
	...
	try: movie_list = json.loads(get_setting(main_setting[0]))
	except: movie_list = []
	try: tvshow_list = json.loads(get_setting(main_setting[1]))
	except: tvshow_list = []
	movie_list_name = get_setting(display_setting[0])
	tvshow_list_name = get_setting(display_setting[1])
	keep_nonlist_items = get_setting('subscriptions.keep_nonlist', 'false')
	...
	if any([update_movies, update_tvshows]):
		...
		def _process_additions_removals(db_type, action, list_contents):
			...
			dialog_db = '[B]%s[/B]' % ls(32028) if db_type in ('movie', 'movies') else '[B]%s[/B]' % ls(32029)
			dialog_line2 = ls(32840) if action == 'add' else ls(32841)
			path = settings.movies_directory() if db_type in ('movie', 'movies') else settings.tv_show_directory()
			threads = []
			list_length = len(list_contents)
			dialog.update(0, dialog_db, dialog_line2 % list_length)
			for count, item in enumerate(list_contents, 1):
		...
		if update_movies:
			...
			if keep_nonlist_items == 'false': remove_from_subscriptions = [i for i in subscriptions_movie_compare_contents if not i in trakt_movie_compare_contents]
			else: remove_from_subscriptions = list()
		...
		if update_tvshows:
			...
			if keep_nonlist_items == 'false': remove_from_subscriptions = [i for i in subscriptions_tvshow_compare_contents if not i in trakt_tvshow_compare_contents]
			else: remove_from_subscriptions = list()
			logger('remove length', remove_from_subscriptions)
			if len(add_to_subscriptions) > 0:
				tvshows_added = True
	...
	dialog.update(100, ls(32577), ls(32845))
	xbmc.sleep(1500)
	dialog.close()
