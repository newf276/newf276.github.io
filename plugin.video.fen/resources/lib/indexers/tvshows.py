...
class TVShows:
	...
	def fetch_list(self):
		try:
			...
			if self.action in ('tmdb_tv_popular','tmdb_tv_top_rated', 'tmdb_tv_premieres','tmdb_tv_upcoming',
				...
				self.sim_recom_name = self.params.get('sim_recom_name')
				self.sim_recom_tmdb = self.params.get('sim_recom_tmdb')
				self.sim_recom_imdb = self.params.get('sim_recom_imdb')
				data, total_pages = function(self.sim_recom_imdb, page_no)
				for item in data: self.list.append(get_trakt_tvshow_id(item['ids']))
				if total_pages > page_no: self.new_page = {'mode': mode, 'action': self.action, 'new_page': str(page_no + 1), 'sim_recom_name': self.sim_recom_name, 'sim_recom_tmdb': self.sim_recom_tmdb, 'sim_recom_imdb': self.sim_recom_imdb, 'foldername': self.action, 'imdb_id': self.params.get('imdb_id')}
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			url_params = {'mode': 'play_action', 'db_type': 'tvshow', 'tmdb_id': tmdb_id, 'total_seasons': total_seasons, 'is_widget': self.is_widget,
						  'show_all_episodes': self.show_all_episodes, 'all_episodes': self.all_episodes, 'meta': meta_json}
			...
			if self.action in ('trakt_tv_related', 'tmdb_tv_recommendations'):
				sim_recom_title = self.similar_str if self.action == 'trakt_tv_related' else self.recomm_str
				export_sim_recom_params = {'mode': 'navigator.adjust_main_lists', 'method': 'add_sim_recom_external', 'end_mode': 'build_tvshow_list',
										   'end_action': self.action, 'db_type': 'tvshow', 'sim_recom_name': '[B]%s[/B] | %s %s' % (self.tv_shows_str, sim_recom_title, self.sim_recom_name),
										   'sim_recom_tmdb': self.sim_recom_tmdb, 'sim_recom_imdb': self.sim_recom_imdb}
				cm.append((self.export_str % sim_recom_title.split(' ')[0], 'RunPlugin(%s)' % build_url(export_sim_recom_params)))
			...
			# 	listitem.setProperty('fen_next_page_params', self.next_page_params)
	...
	def set_meta(self, _id):
		meta = tikimeta.tvshow_meta(self.id_type, _id, self.meta_user_info)
		if not meta: return
	...
	def assign_labels(self):
		self.watched_str, self.unwatched_str, self.addremove_str, self.extras_str, self.options_str, self.export_str = ls(32642), ls(32643), ls(32644), ls(32645), ls(32646), ls(32647)
		self.hide_str, self.exit_str, self.similar_str, self.recomm_str, self.tv_shows_str, self.browse_str = ls(32648), ls(32650), ls(32672), ls(32673), ls(32029), ls(32652)
	...
	def _process():
		for item in episodes_data:
			try:
				...
				url_params = {'mode': 'play_action', 'db_type': 'episode', 'tmdb_id': tmdb_id,
							'query': query, 'tvshowtitle': meta['rootname'], 'season': season,
							'episode': episode, 'meta': meta_json, 'is_widget': is_widget}
				url = build_url(url_params)
				display = ep_name
				unaired = False
