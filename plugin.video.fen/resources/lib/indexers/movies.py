...
class Movies:
	...
	def build_movie_content(self, item_position, _id):
		try:
			...
			url_params = {'mode': 'play_action', 'db_type': 'movie', 'tmdb_id': tmdb_id, 'query': rootname, 'meta': meta_json, 'is_widget': self.is_widget}
			...
			if self.action in ('trakt_movies_related', 'tmdb_movies_recommendations'):
				sim_recom_title = self.similar_str if self.action == 'trakt_movies_related' else self.recomm_str
				export_sim_recom_params = {'mode': 'navigator.adjust_main_lists', 'method': 'add_sim_recom_external', 'end_mode': 'build_movie_list',
										   'end_action': self.action, 'db_type': 'movie', 'sim_recom_name': '[B]%s[/B] | %s %s' % (self.movies_str, sim_recom_title, self.sim_recom_name),
	...
	def set_meta(self, _id):
		meta = tikimeta.movie_meta(self.id_type, _id, self.meta_user_info)
		if not meta: return
	...
	def worker(self):
		...
		self.open_action = settings.default_open_action('movie')
	...
	def assign_labels(self):
		...
		self.hide_str, self.exit_str, self.clearprog_str, self.similar_str, self.recomm_str, self.movies_str = ls(32648), ls(32649), ls(32651), ls(32672), ls(32673), ls(32028)
