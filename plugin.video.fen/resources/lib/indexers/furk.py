...
from modules.settings import get_theme, include_uncached_results
...
def torrent_status_browser(files):
	...
	for count, item in enumerate(files, 1):
		try:
			display = '%02d | %s | [COLOR=grey][I]%s | %sGB | %s %% | %s: %s kB/s | (S:%s P:%s)[/I][/COLOR]' % (count, item['name'].replace('magnet:', '').upper(), item['dl_status'].upper(), str(round(float(item['size'])/1048576000, 1)), item['have'], ls(32775), str(round(float(item['speed'])/1024, 1)), item['seeders'], item['peers'])
	...
	_cache.set('furk_active_downloads', [i['info_hash'] for i in files], expiration=timedelta(hours=1))
	xbmcplugin.setContent(__handle__, 'files')
	xbmcplugin.endOfDirectory(__handle__, cacheToDisc=True)
	setView('view.premium')
...
def search_furk(params):
	...
	if not search_title: search_title = dialog.input('Fen', type=xbmcgui.INPUT_ALPHANUM, defaultt=default)
	...
	try:
		...
		if not include_uncached_results():
			try: files = [i for i in files if i.get('is_ready', '0') == '1' and i['type'] == list_type]
			except: return dialog.ok('Fen', ls(32760))
		files = files[0:int(get_setting('furk.limit'))]
...
def furk_tfile_audio(params):
	...
	if not item_path_list:
		if dialog.yesno("Fen", ls(32763), '', '', ls(32764), ls(32652)):
...
def furk_file_browser(files, params, display_mode, __handle__):
	...
	active_str, uncached_str, files_str, down_str, add_str, remove_str, prot_str, unprot_str = ls(32489).upper(), ls(32765).upper(), ls(32493).upper(), ls(32747), ls(32769), ls(32766), ls(32767), ls(32768)
	for count, item in enumerate(files, 1):
		try:
			...
			if uncached:
				...
				if item['info_hash'] in active_downloads:
					info = '%02d | [COLOR=green][B]%s[/B][/COLOR] |' % (count, active_str)
				else:
					info = '%02d | [COLOR=red][B]%s[/B][/COLOR] |' % (count, uncached_str)
			else: mode = orig_mode
			name = clean_file_name(item['name']).upper()
			...
			if not uncached:
				...
				if display_mode == 'search': cm.append((add_str,'RunPlugin(%s)' % build_url(con_add_to_files)))
				cm.append((down_str,'RunPlugin(%s)' % build_url(con_download_archive)))
				cm.append((remove_str,'RunPlugin(%s)' % build_url(con_remove_files)))
				if is_protected == '0': cm.append((prot_str,'RunPlugin(%s)' % build_url(con_protect_files)))
				if is_protected == '1': cm.append((unprot_str,'RunPlugin(%s)' % build_url(con_unprotect_files)))
			listitem = xbmcgui.ListItem(display)
			listitem.addContextMenuItems(cm)
			listitem.setArt({'icon': default_furk_icon, 'poster': default_furk_icon, 'thumb': default_furk_icon, 'fanart': fanart, 'banner': default_furk_icon})
			xbmcplugin.addDirectoryItem(__handle__, url, listitem, isFolder=True)
...
def remove_from_downloads(item_id):
	...
	if resp:
		...
		if response['status'] == 'ok':
			from datetime import timedelta
			_cache.set('furk_active_downloads', None, expiration=timedelta(hours=1))
			notification(ls(32576), 3500)
		else:
			notification(ls(32574), 3500)
		return (None, None)
...
def add_uncached_file(item_id):
	if not dialog.yesno('Fen', ls(32580)):
		return
	try:
...
def account_info(params):
	try:
		...
		heading = 'FURK'
		body = []
		body.append(ls(32758) % account_type.upper())
		body.append(ls(32770) % str(round(bw_limit_month, 0)))
		body.append(ls(32771))
		body.append('        - %s' % ls(32751) % str(round(month_time_left, 2)))
		body.append('        - %s GB' % ls(32761) % str(round(bw_used_month, 2)))
		body.append('        - %s GB' % ls(32762) % str(round(rem_bw_limit_month, 2)))
		if not account_type == 'LIFETIME':
			body.append(ls(32772))
			body.append('[B]        - %s' % ls(32751) % str(round(total_time_left, 0)))
			if is_not_last_month == '1': body.append('        - %s' % ls(32773) % renewal_date)
			else: body.append('        - %s' % ls(32774) % renewal_date)
