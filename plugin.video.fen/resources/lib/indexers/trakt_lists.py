...
def search_trakt_lists(params):
	...
	for count, item in enumerate(lists, 1):
		try:
			...
			cm.append((ls(32777),'RunPlugin(%s)' % build_url(trakt_subscriptions_url)))
...
def get_trakt_list_selection(list_choice='none'):
	...
	if list_choice == 'subscriptions':
		my_lists.insert(0, {'name': 'NONE', 'display': '[B]%s[/B]' % ls(32965), 'user': 'NONE', 'slug': 'NONE'})
	...
	def _process_my_lists():
		for item in lists:
			try:
				...
				cm.append((ls(32782),'RunPlugin(%s)' % build_url(trakt_subscriptions_url)))
