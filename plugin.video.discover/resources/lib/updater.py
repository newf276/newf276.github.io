...
def update_players(path):
    if path.startswith('http://') or path.startswith('https://'): return update_players_remote(path)
    return update_players_local(path)
    
def update_players_local(zip_path):
    extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.discover/Players')  
...
def update_players_remote(url):
    ...
    if username is not None:
        if not password: password = plugin.keyboard(heading='Enter password', hidden=True)
...
def flat_extract(z, extract_to, members=None):
    if members is None: members = z.namelist()
    if not os.path.exists(extract_to): os.makedirs(extract_to)
    else:
        if xbmc.getInfoLabel('Window(home).Property(running)') == 'totalmeta': empty_folder(extract_to)
        else:
            if dialogs.yesno('Discover: Update players', 'Do you want to remove existing players first?'): empty_folder(extract_to)
    for member in members:
        with contextlib.closing(z.open(member)) as source:
            target_path = os.path.join(extract_to, os.path.basename(member))
            with open(target_path, 'wb') as target: shutil.copyfileobj(source, target)
