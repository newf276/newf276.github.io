...
def add_movie_to_library(library_folder, src, id):
    ...
    if not xbmcvfs.exists(nfo_filepath):
        ...
        if src == 'imdb': content = 'https://www.imdb.com/title/%s/' % str(id)
        else: content = 'https://www.themoviedb.org/movie/%s' % str(id)
...
def setup_library(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = 'Would you like to automatically set Discover as a movies video source?'
        if dialogs.yesno('Library setup', msg):
            source_thumbnail = nav_base.get_icon_path('movies')
            source_name = 'Discover Movies'
