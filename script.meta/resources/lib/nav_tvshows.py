...
def make_tvshow_item(info):
    ...
    if xbmc.getCondVisibility('system.hasaddon(script.openinfo)'):
        context_menu = [('OpenInfo', 'RunScript(script.openinfo,info=extendedtvinfo,tvdb_id=%s)' % tvdb_id),
                        ('TV trailer', 'RunScript(script.openinfo,info=playtvtrailer,tvdb_id=%s)' % tvdb_id),
...
def list_seasons_tvdb(id):
    ...
    for (season_num, season) in show.items():
        ...
        if xbmc.getCondVisibility('system.hasaddon(script.openinfo)'):
            context_menu = [('OpenInfo', 'RunScript(script.openinfo,info=seasoninfo,tvshow=%s,season=%s)' % (title, season_num)),
                            ('TV trailer', 'RunScript(script.openinfo,info=playtvtrailer,tvdb_id=%s)' % id)]
        else: context_menu = []
...
def list_episodes_tvdb(id, season_num):
    ...
    for (episode_num, episode) in season.items():
        ...
        if xbmc.getCondVisibility('system.hasaddon(script.openinfo)'):
            context_menu = [('OpenInfo', 'RunScript(script.openinfo,info=extendedepisodeinfo,tvshow=%s,season=%s,episode=%s)' % (title, season_num, episode_num)),
                            ('TV trailer', 'RunScript(script.openinfo,info=playtvtrailer,tvdb_id=%s)' % id)]
        else: context_menu = []
