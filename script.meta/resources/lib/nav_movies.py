...
def movies():
    items = [
        {'label': 'Genres (TMDb)', 'path': plugin.url_for('tmdb_movies_genres'), 'icon': nav_base.get_icon_path('genres')
        },
        {'label': 'Blockbusters (TMDb)', 'path': plugin.url_for('tmdb_movies_blockbusters', page='1'), 'icon': nav_base.get_icon_path('most_voted'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('tmdb_movies_play_random_blockbuster'))]
        },
        {'label': 'In theatres (TMDb)', 'path': plugin.url_for('tmdb_movies_now_playing', page='1'), 'icon': nav_base.get_icon_path('intheatres'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('tmdb_movies_play_random_now_playing'))]
        },
        {'label': 'Popular (TMDb)', 'path': plugin.url_for('tmdb_movies_popular', page='1'), 'icon': nav_base.get_icon_path('popular'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('tmdb_movies_play_random_popular'))]
        },
        {'label': 'Top rated (TMDb)', 'path': plugin.url_for('tmdb_movies_top_rated', page='1'), 'icon': nav_base.get_icon_path('top_rated'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('tmdb_movies_play_random_top_rated'))]
        },
        {'label': 'Most watched (Trakt)', 'path': plugin.url_for('trakt_movies_watched', page='1'), 'icon': nav_base.get_icon_path('traktwatchlist'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_watched'))]
        },
        {'label': 'Most collected (Trakt)', 'path': plugin.url_for('trakt_movies_collected', page='1'), 'icon': nav_base.get_icon_path('traktcollection'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_collected'))]
        },
        {'label': 'Popular (Trakt)', 'path': plugin.url_for('trakt_movies_popular', page='1'), 'icon': nav_base.get_icon_path('traktrecommendations'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_popular'))]
        },
        {'label': 'Trending (Trakt)', 'path': plugin.url_for('trakt_movies_trending', page='1'), 'icon': nav_base.get_icon_path('trending'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_trending'))]
        },
        {'label': 'Latest releases (Trakt)', 'path': plugin.url_for('trakt_movies_latest_releases'), 'icon': nav_base.get_icon_path('traktcalendar'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_latest_releases'))]
        },
        {'label': 'Top 250 (IMDB)', 'path': plugin.url_for('trakt_movies_imdb_top_rated', page='1'), 'icon': nav_base.get_icon_path('imdb'),
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('trakt_movies_play_random_imdb_top_rated'))]
        },
        {'label': 'Search movies', 'path': plugin.url_for('movies_search'), 'icon': nav_base.get_icon_path('search')}]
...
def tmdb_movies_play_random_blockbuster():
    ...
    for i in range(1, pages): result.update(tmdb_movies_blockbusters(i, raw=True))
...
def tmdb_movies_play_random_now_playing():
    result = {}
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.update(tmdb_movies_now_playing(i, raw=True))
    tmdb_movies_play_random(result)
...
def tmdb_movies_play_random_popular():
    result = {}
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.update(tmdb_movies_popular(i, raw=True))
    tmdb_movies_play_random(result)
...
def tmdb_movies_play_random_top_rated():
    result = {}
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.update(tmdb_movies_top_rated(i, raw=True))
    tmdb_movies_play_random(result)
...
def movies_search_term(term, page):
    items = [
        {'label': '(TMDb) Search - %s' % term, 'path': plugin.url_for('tmdb_movies_search_term', term=term, page='1'), 'icon': nav_base.get_icon_path('movies')},
        {'label': '(Trakt) Search - %s' % term, 'path': plugin.url_for('trakt_movies_search_term', term=term, page='1'), 'icon': nav_base.get_icon_path('movies')},
        {'label': 'Edit search string', 'path': plugin.url_for('movies_search_edit', term=term), 'icon': nav_base.get_icon_path('search')}]
    for item in items: item['properties'] = {'fanart_image': nav_base.get_background_path()}
    return items
...
def trakt_movies_play_random_imdb_top_rated():
    result = []
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.extend(trakt_movies_imdb_top_rated(i, raw=True))
...
def trakt_movies_play_random_watched():
    result = []
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.extend(trakt_movies_watched(i, raw=True))
    trakt_movies_play_random(result)
...
def trakt_movies_play_random_collected():
    result = []
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.extend(trakt_movies_collected(i, raw=True))
    trakt_movies_play_random(result)
...
def trakt_movies_play_random_popular():
    result = []
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.extend(trakt_movies_popular(i, raw=True))
    trakt_movies_play_random(result)
...
def tmdb_movies_genres():
    ...
    items = sorted([
        {'label': name,
         ...
         'context_menu': [('Play (random)', 'RunPlugin(%s)' % plugin.url_for('tmdb_movies_play_random_genre', id = id))]} for id, name in genres.items()], key=lambda k: k['label'])
    for item in items: item['properties'] = {'fanart_image': nav_base.get_background_path()}
...
def tmdb_movies_play_random_genre(id):
    result = {}
    pages = plugin.get_setting(settings.SETTING_RANDOM_PAGES, int) + 1
    for i in range(1, pages): result.update(tmdb_movies_genre(id, i, raw=True))
    tmdb_movies_play_random(result)
...
def movies_add_all_to_library(items, noscan=False):
    ...
    batch_add_file.close()
    xbmc.executebuiltin('RunPlugin(plugin://script.meta/movies/batch_add_to_library)')
...
def movies_batch_add_to_library():
    ...
    if xbmcvfs.exists(movie_batch_file):
        try:
            ...
            return dialogs.notify(title='Movies', msg=title, delay=2000, image=nav_base.get_icon_path('meta'))
        library_folder = lib_movies.setup_library(plugin.get_setting(settings.SETTING_MOVIES_LIBRARY_FOLDER, unicode))
        for id in ids:
            if ',' in id:
                csvs = id.split(',')
...
def list_tmdb_movies(result):
    ...
    if 'page' in result:
        ...
        if pages > page:
            args['page'] = page + 1
            args['confirm'] = 'yes'
            items.append({'label': '%s/%s  [I]Next page[/I]  >>' % (page, pages + 1),
...
def movies_play(src, id):
    ...
    if tmdb_id: play_movies.play_movie(tmdb_id)
    else: plugin.set_resolved_url(handle=int(sys.argv[1]))
...
def movies_play_by_name(name, lang='en'):
    ...
    if len(items) > 1: selection = dialogs.select(('movie'), ['%s (%s)' % ((s['title']), text.parse_year(s['release_date'])) for s in items])
    else: selection = 0
    if selection != -1:
...
def trakt_movies_play_random(movies, convert_list=False):
    for movie in movies:
        movie['type'] = 'movie'
        if convert_list: movie['movie'] = movie
...
def make_movie_item(movie_info):
    ...
    if xbmc.getCondVisibility('system.hasaddon(script.openinfo)'):
        context_menu = [('OpenInfo', 'RunScript(script.openinfo,info=extendedinfo,id=%s)' % id),
                        ('Movie trailer', 'RunScript(script.openinfo,info=playtrailer,id=%s)' % id),
